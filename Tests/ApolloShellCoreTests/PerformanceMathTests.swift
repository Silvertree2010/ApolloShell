...
@Suite("Performance logic")
struct PerformanceMathTests {
    // MARK: Network

    @Test("Rate from two counter readings")
    func rate() {
    ...
    @Test("Beyond 4 GB the 64-bit counters just keep counting")
    ...
    @Test("Reset counter or no time in between: nil")
    func rateDegenerate() {
    ...
    @Test("Only real interfaces count: no loopback, no VPN tunnel, no bridge")
    ...
    @Test("Meter: first measurement without a rate, then rate and total")
    func meterBasics() {
    ...
    @Test("Meter: no rate after the pause, the total keeps running")
    func meterPause() {
        var meter = NetworkMeter()
        ...
        #expect(meter.rate == nil) // averaged over a 60 s pause would be wrong
        #expect(meter.total.received == 61_000)
        meter.add(NetCounters(received: 62_000, sent: 0), at: 62)
        #expect(meter.rate?.download == 1_000)
    }

    @Test("Meter: counter drops (VPN gone) - no jump, then it continues")
    func meterReset() {
        var meter = NetworkMeter()
    ...
    @Test("History keeps only the last 30 values, oldest first")
    ...
    @Test("Sparkline: right-aligned, normalized to the scale, clamped")
    ...
    @Test("Scale: maximum of the history, but at least the floor")
    func sparklineScale() {
    ...
    @Test("Bytes in German, decimal", arguments: [
        (0.0, "0 B"),
        (999.0, "999 B"),
        (999.6, "1,0 KB"),       // rounded 1000 B -> next unit
        (1_234.0, "1,2 KB"),
        (340_000.0, "340 KB"),
        (999_700.0, "1,0 MB"),
        (1_234_567.0, "1,2 MB"),
        (9_960_000.0, "10 MB"),  // 9.96 -> no decimal place anymore
    ...
    @Test("Bytes in English: point instead of comma")
    func bytesEnglish() {
    ...
    @Test("Memory binary: 24 GB stays 24 GB")
    func binaryUsage() {
        let gib: UInt64 = 1 << 30
    ...
    @Test("Percent with a dash when there is no measurement")
    func percent() {
    ...
    @Test("Battery tank: fill level and texts")
    func batteryTank() {
