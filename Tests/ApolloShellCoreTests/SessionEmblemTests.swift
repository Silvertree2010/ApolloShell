...
@Suite("Emblem in the session menu")
struct SessionEmblemTests {
    @Test("Reaction to a button under the mouse or selected", arguments: [
    ...
    @Test("After a one-shot movement, the resting reaction stays", arguments: [
        (SessionAction?.none, EmblemReaction.idle),
        (SessionAction.sleep, EmblemReaction.sleep),
    ...
    @Test("Only greeting and farewell are one-shot", arguments: [
    ...
    @Test("A one-shot movement reports finished only at the end", arguments: [EmblemReaction.greet, .farewell])
    ...
    @Test("A one-shot movement ends in the resting pose with resting speed", arguments: [EmblemReaction.greet, .farewell])
    ...
    @Test("Loops start in the resting pose", arguments: [EmblemReaction.idle, .sleep, .farewell, .think])
    ...
    @Test("At rest the moon stays still, night is fully in effect", arguments: [3.0, 10.0])
    ...
    @Test("The moon doesn't jump when switching", arguments: [
    ...
    @Test("The same reaction again doesn't restart", arguments: [EmblemReaction.idle, .think])
    ...
    @Test("Reduced motion: fixed pose, the moon always in the same place", arguments: EmblemReaction.allCases)
    ...
    @Test("Still poses keep their meaning", arguments: [
    ...
    @Test("Menu bezier curve: endpoints and overshoot", arguments: [0.0, 1.0])
    func menuCurveEnds(x: Double) {
        #expect(abs(EmblemPose.menuCurve(x) - x) < 1e-6)
    }

    @Test("Bezier curve overshoots slightly", arguments: [0.6])
