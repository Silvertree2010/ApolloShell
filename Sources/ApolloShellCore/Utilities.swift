...
public enum KeepAwakeText {
    public static let title = String(localized: "Wach halten")
    public static let inactive = String(localized: "Mac schläft normal")
    ...
    public static func subtitle(since: Date?, now: Date, lid: KeepAwakeLid, calendar: Calendar = .current) -> String {
        ...
        case .on: return base + String(localized: " · auch zugeklappt")
        case .pending: return base + String(localized: " · wartet auf Freigabe")
        case .declined: return base + String(localized: " · nur aufgeklappt")
        }
    }
    ...
    private static func plainSubtitle(since: Date, now: Date, calendar: Calendar) -> String {
        ...
        if calendar.isDate(since, inSameDayAs: now) {
            return String(localized: "Aktiv seit \(time)")
        }
        if let yesterday = calendar.date(byAdding: .day, value: -1, to: now),
           calendar.isDate(since, inSameDayAs: yesterday) {
            return String(localized: "Aktiv seit gestern, \(time)")
        }
        return String(localized: "Aktiv seit \(String(format: "%02d.%02d.", c.day ?? 0, c.month ?? 0)), ") + time
    }
}
...
public enum QuickToggles {
    ...
    public static func wifi(powerOn: Bool?) -> QuickToggleLook {
        switch powerOn {
        case true?: QuickToggleLook(symbol: "wifi", active: true, enabled: true, help: String(localized: "WLAN an"))
        case false?: QuickToggleLook(symbol: "wifi.slash", active: false, enabled: true, help: String(localized: "WLAN aus"))
        case nil: QuickToggleLook(symbol: "wifi.slash", active: false, enabled: false, help: String(localized: "Kein WLAN gefunden"))
        }
    }
    ...
    public static func microphone(muted: Bool?, settable: Bool) -> QuickToggleLook {
        guard let muted else {
            return QuickToggleLook(symbol: "mic.slash", active: false, enabled: false, help: String(localized: "Kein Mikrofon"))
        }
        let state = muted ? String(localized: "Mikrofon stumm") : String(localized: "Mikrofon an")
        return QuickToggleLook(
            symbol: muted ? "mic.slash.fill" : "mic.fill",
            active: !muted,
            enabled: settable,
            help: settable ? state : state + String(localized: " (nicht schaltbar)")
        )
    }
    ...
    public static func bluetooth(powerOn: Bool?) -> QuickToggleLook {
        ...
        return QuickToggleLook(symbol: nil, active: powerOn == true, enabled: true,
                               help: state + String(localized: " – Einstellungen öffnen"))
    }
    ...
    public static let settings = QuickToggleLook(
        symbol: "gearshape.fill", active: false, enabled: true, help: String(localized: "Einstellungen (SUPER+,)")
    )
    ...
    public static func darkMode(on: Bool?) -> QuickToggleLook {
        switch on {
        case true?: QuickToggleLook(symbol: "circle.lefthalf.filled", active: true, enabled: true, help: String(localized: "Dunkelmodus an"))
        case false?: QuickToggleLook(symbol: "circle.lefthalf.filled", active: false, enabled: true, help: String(localized: "Dunkelmodus aus"))
        case nil: QuickToggleLook(symbol: "circle.lefthalf.filled", active: false, enabled: false,
                                  help: String(localized: "Dunkelmodus nicht lesbar"))
        }
    }
    ...
    public static func nightShift(enabled: Bool?) -> QuickToggleLook {
        switch enabled {
        case true?: QuickToggleLook(symbol: "sunset.fill", active: true, enabled: true, help: String(localized: "Night Shift an"))
        case false?: QuickToggleLook(symbol: "sunset.fill", active: false, enabled: true, help: String(localized: "Night Shift aus"))
        case nil: QuickToggleLook(symbol: "sunset.fill", active: false, enabled: false, help: String(localized: "Night Shift nicht verfügbar"))
        }
    }

    /// Aktionen leuchten nie, sie haben keinen Zustand.
    public static let screenshot = QuickToggleLook(
        symbol: "camera.viewfinder", active: false, enabled: true, help: String(localized: "Bildschirmfoto oder Aufnahme (⌘⇧5)")
    )
    ...
    public static func showDesktop(available: Bool) -> QuickToggleLook {
        QuickToggleLook(
            ...
            help: available ? String(localized: "Schreibtisch anzeigen") : String(localized: "Schreibtisch anzeigen – Kurzbefehl in Mission Control aus")
        )
    }

    public static let colorPicker = QuickToggleLook(
        symbol: "eyedropper", active: false, enabled: true, help: String(localized: "Farbpipette – Hexwert in die Zwischenablage")
    )

    public static let lockScreen = QuickToggleLook(
        symbol: "lock.fill", active: false, enabled: true, help: String(localized: "Bildschirm sperren (⌃⌘Q)")
    )
